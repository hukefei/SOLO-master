...
class EfficientNet(nn.Module):
    ...
    def __init__(self,
        ...
        for block_args in self._blocks_args:
            ...
            self.stage_blocks_num.append(block_args.num_repeat)
            if block_args.num_repeat > 1:
                block_args = block_args._replace(input_filters=block_args.output_filters, stride=1)
            for _ in range(block_args.num_repeat - 1):
                self._blocks.append(MBConvBlock(block_args, self._global_params))

        self.stage_last_indices = list(np.cumsum(self.stage_blocks_num) - 1)
        self.out_inds = [self.stage_last_indices[i] for i in self.out_indices]
    ...
    def _freeze_stages(self):
        ...
        if self.frozen_stages >= 1:
            frozen_layers = self.stage_last_indices[self.frozen_stages - 1]
            for idx, m in enumerate(self._blocks):
                if idx in range(frozen_layers):
                    m.eval()
                    for param in m.parameters():
                        param.requires_grad = False
